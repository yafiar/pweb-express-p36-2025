...
import {
  createBookSchema, updateBookSchema, listQuerySchema
} from './books.validator';
import {
  createBook, getBookById, listBooks, listBooksByGenre, updateBook, deleteBookSafe
} from './books.service';
...
export async function createBookHandler(req: Request, res: Response) {
  try {
    ...
    // Unique title requirement → conflict
    if ((err as any)?.code === 'P2002') {
      return res.status(409).json({ message: 'Book title already exists' }); // unique title
    }
